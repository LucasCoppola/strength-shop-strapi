...
const ProductCard = ({ product }: { product: Product }) => {
	...
	return (
		<a title={product.name} className="group rounded-lg border">
			<div className="aspect-h-1 aspect-w-1 xl:aspect-h-8 xl:aspect-w-7 relative w-full overflow-hidden">
				<img src={image} alt={product.name} className="h-full w-full object-cover object-center group-hover:opacity-50" />
				<button
					title="Add to cart"
